 ...
 *
 *        Содержит основные идентификационные данные устройства, полученные
 ...
 *
 *        Наследует от std::runtime_error и добавляет код ошибки FT_STATUS
 ...
 *
 * Предоставляет статический метод для получения списка всех подключенных
 ...
 *
 * Реализует высокоуровневый интерфейс для работы с чипом FT4222H,
...
public:
     ...
     *
     * Определяет текущий активный режим работы устройства.
     ...
     *
     * Определяет направление каждого GPIO вывода при инициализации.
     */
    enum class GPIOMode {
     ...
     *
     * Стандартные скорости I2C шины. Реальная скорость может отличаться
     ...
     *
     * Определяют коэффициент деления системной частоты для генерации
     ...
     * @return Если устройство открыто и готово к работе, то true
     */
